...
ref<Scene> UsdImporter::loadScene(const std::string& fileName)
{
    ...
    scene->name = fileName;
    scene->materials.push_back(Material()); // Add a default material
    ...
    // Extract cameras first
    extractCameras(rootXformNode, scene);
    ...
    LOG_INFO(
        "Scene conversion completed. Found {} vertices, {} indices, {} meshes", scene->vertices.size(), scene->indices.size(), scene->meshes.size()
    );
    return scene;
}
...
void UsdImporter::traverseXformNode(const tinyusdz::tydra::XformNode& node, ref<Scene> scene)
{
    ...
    if (node.prim)
    {
        // Check if this prim is a GeomMesh
...
void UsdImporter::extractCameras(const tinyusdz::tydra::XformNode& node, ref<Scene> scene)
{
    // Check if this node has a valid prim
    if (node.prim)
    {
        // Check if this prim is a GeomCamera
        if (const tinyusdz::GeomCamera* geomCamera = node.prim->as<tinyusdz::GeomCamera>())
        {
            LOG_INFO("Found camera: {}", geomCamera->name);
            const tinyusdz::value::matrix4d& worldMatrix = node.get_world_matrix();
    ...
    // Recursively traverse children to find cameras
    for (const auto& child : node.children)
        extractCameras(child, scene);
}
...
void UsdImporter::extractMeshGeometry(const tinyusdz::GeomMesh& geomMesh, const tinyusdz::value::matrix4d& worldMatrix, ref<Scene> scene)
{
    ...
    // Create mesh entry
    Mesh mesh;
    mesh.materialIndex = 0; // Default material for now

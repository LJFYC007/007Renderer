...
int RenderGraphEditor::findOutputPinId(const std::string& passName, const std::string& outputName)
{
    ...
    auto nodeIt = std::find_if(mEditorNodes.begin(), mEditorNodes.end(), [&passName](const RenderGraphNode& n) { return n.name == passName; });
    if (nodeIt == mEditorNodes.end())
        return -1;
        ...
        for (const auto& [passName, pinIds] : mOutputPinIds)
        {
            for (size_t i = 0; i < pinIds.size(); ++i)
            {
                if (pinIds[i] == pinId.Get())
                {
                    ...
                    auto nodeIt =
                        std::find_if(mEditorNodes.begin(), mEditorNodes.end(), [&passName](const RenderGraphNode& n) { return n.name == passName; });

...
Window::Window(ComPtr<ID3D12Device> device, ComPtr<ID3D12CommandQueue> commandQueue, const desc& windowDesc)
{
...
{ // Setup swap chain
    ...
    if (g_pd3dDevice->CreateCommandList(
            0, D3D12_COMMAND_LIST_TYPE_DIRECT, g_frameContext[0].CommandAllocator, nullptr, IID_PPV_ARGS(&g_pd3dCommandList)
        ) != S_OK ||

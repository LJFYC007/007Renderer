...
//------------------------------------------------------------------------------
static inline void ImRectFloor(ImRect& r) { r.Min = ImFloor(r.Min); r.Max = ImFloor(r.Max); }
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control& control)
{
    ...
    if (Editor->CanAcceptUserInput() && ImGui::IsKeyDown(GetKeyIndexForF()) && Editor->AreShortcutsEnabled())
    {
